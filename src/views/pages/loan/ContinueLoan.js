...
const CreateLoan = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [customerResponse, termsResponse, depositTypeResponse, categoryResponse, loanTypeResponse] = await Promise.all([
        Axios.get(process.env.REACT_APP_API_ENDPOINT + '/customer/all', { headers: { 'x-token': getToken() } }),
        ...
        fields: [
          {
            ...
            formField: {
              ...
              type: 'search', // select | TextField | file | email | phonenumber | number | hidden | textarea | password
              xs: 6,
              value: '',
              isRequired: true,
              validationType: 'default' // default | custom
            },
            editSelectOptions: customer.map((customer) => ({
  ...
  const createLoanForm = useMemo(
    ...
    [customer, category, loanType, terms]
  );
